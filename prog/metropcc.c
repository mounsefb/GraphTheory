...
int main(int argc, char**argv) {
  ...
  if(argv[3][0]>=48 && argv[3][0]<=57 ){
    ...
    departmetro=argv[3];
    arriveemetro=argv[4];
    hashtable_t t= hashtable_new(30);
    graph = graph_read(argv[1], 1, &t);
    //printf("%d, %d\n", hashing(departmetro, t), hashing(arriveemetro,t));
    if(!hashtable_get_value(departmetro, &depart, t) || !hashtable_get_value(arriveemetro, &arrivee, t) ){
      printf("une des stations n'existe pas dans notre dictionnaire \n");
      exit(EXIT_FAILURE);
    }
    hashtable_delete(t);
  }
